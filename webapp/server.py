...
- 날짜/시각은 도구 결과 또는 시스템이 제공한 현재 기준 시각만 사용하고, 확인되지 않은 조회 시각·생성 시각을 임의로 만들지 말 것
"""
...
- 데이터 조회 시각: (시스템이 제공한 현재 기준 시각 — 한국 시각. 값이 없으면 이 줄은 생략)
...
def _build_system(mode: Literal["guide", "report", "stats"], pinned_group: str | None) -> str:
    ...
    full = (
        SYSTEM_BASE
        + f"\n\n## 현재 기준 시각\n- 현재 시각: {now_kst}\n"
        + "- 답변의 데이터 조회 시각이 필요하면 위 현재 시각만 사용하세요.\n"
        + "- 도구 결과에 없는 과거/미래 날짜나 시각은 생성하지 마세요.\n"
        + mode_text
    )
    if pinned_group:
        full += (
